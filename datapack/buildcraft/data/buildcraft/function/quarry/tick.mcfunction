execute as @e[type=minecraft:marker,tag=buildcraft.quarry] at @s unless entity @e[type=minecraft:marker,tag=buildcraft.quarry_head,distance=..1] run function buildcraft:quarry/activate
